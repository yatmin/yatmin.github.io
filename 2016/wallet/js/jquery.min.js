...
}return offsetParent||document.documentElement})}});jQuery.each({scrollLeft:"pageXOffset",scrollTop:"pageYOffset"},function(method,prop){var top=/Y/.test(prop);jQuery.fn[method]=function(val){return jQuery.access(this,function(elem,method,val){var win=getWindow(elem);if(val===undefined){return win?(prop in win)?win[prop]:win.document.documentElement[method]:elem[method]}if(win){win.scrollTo(!top?val:jQuery(win).scrollLeft(),top?val:jQuery(win).scrollTop())}else{elem[method]=val}},method,val,arguments.length,null)}});function getWindow(elem){return jQuery.isWindow(elem)?elem:elem.nodeType===9?elem.defaultView||elem.parentWindow:false}jQuery.each({Height:"height",Width:"width"},function(name,type){jQuery.each({padding:"inner"+name,content:type,"":"outer"+name},function(defaultExtra,funcName){jQuery.fn[funcName]=function(margin,value){var chainable=arguments.length&&(defaultExtra||typeof margin!=="boolean"),extra=defaultExtra||(margin===true||value===true?"margin":"border");return jQuery.access(this,function(elem,type,value){var doc;if(jQuery.isWindow(elem)){return elem.document.documentElement["client"+name]}if(elem.nodeType===9){doc=elem.documentElement;return Math.max(elem.body["scroll"+name],doc["scroll"+name],elem.body["offset"+name],doc["offset"+name],doc["client"+name])}return value===undefined?jQuery.css(elem,type,extra):jQuery.style(elem,type,value,extra)},type,chainable?margin:undefined,chainable,null)}})});window.jQuery=window.$=jQuery;if(typeof define==="function"&&define.amd&&define.amd.jQuery){define("jquery",[],function(){return jQuery})}})(window);$.log=function(){};$.alert=function(v){alert(v)};function getBasePath(){if(location.href.indexOf("/NewRoot/")>-1){return"/NewRoot"}return""}function showLoad(os){os=os?$(os):os=$("span[id^='|load|']");if(os.length<1){return}os.each(function(i){os.get(i).id=os.get(i).id.replace(/\|load\|/,"|")});window.addLazy(new lazyObj(os,function(o){var url=o.attr("id").replace(/\|.+\|/,"").replace(/^\|/,"");o.load(url);o.show()}))}function viewDo(os){os=os?$(os):$("[_type='viewDo']");if(os.length<1){return}os.each(function(i){$(os.get(i)).attr("_type","")});window.addLazy(new lazyObj(os,function(o){var url=o.attr("_load"),fun=o.attr("_loadOver"),m=o.attr("_m"),suc=o.attr("suc");if(url){fun=(fun==null)?"":fun.replace("[this]","o");$.ajax({url:url,async:true,dataType:"html",type:"get",success:function(req){if(suc){eval(suc.replace("[this]","o"))}else{o.html(req)}eval(fun)}})}if(m){eval(m.replace("[this]","o"))}o.show()}))}function showAttr(os){if(!os){os=$("[id*='|attr|']")}os=$(os);if(os.length<1){return}os.each(function(i){os.get(i).id=os.get(i).id.replace(/\|attr\|/,"|");$(os.get(i)).html("<span height='25'><img name='"+os.get(i).innerHTML+"' src='"+getBasePath()+"/images/loading.gif'/></span>")});window.addLazy(new lazyObj(os,function(o){var toUserName=o.find("img").attr("name");var param=getBasePath()+"/ajax/atten/view.jsp?m="+$.now()+"&toUser="+encodeURIComponent(toUserName);o.load(param);o.show()}))}function showHeadPic(os){if(!os){os=$("img[id*='|head|']")}os=$(os);if(os.length<1){return}os.each(function(i){os.get(i).id=os.get(i).id.replace(/\|head\|/,"|");os.get(i).src=getBasePath()+"/images/headImgLoading.gif"});window.addLazy(new lazyObj(os,function(o){o.attr("src",o.attr("_src"));o.removeAttr("_src")},260))}function showMedalPic(os){if(!os){os=$("img[id*='|medal|']")}os=$(os);if(os.length<1){return}os.each(function(i){os.get(i).id=os.get(i).id.replace(/\|medal\|/,"|");os.get(i).src=getBasePath()+"/images/headImgLoading.gif"});window.addLazy(new lazyObj(os,function(o){o.attr("src",o.attr("_src"));o.removeAttr("_src")},200))}$(document).ready(function(){window.doLazy={};window.addLazy=function(lazyObj){if(lazyObj){if(window.doLazy[lazyObj.id]){window.doLazy[lazyObj.id].addRunValue(lazyObj.baselist)}else{window.doLazy[lazyObj.id]=lazyObj}}};$(window).bind("scroll",function(){for(var key in window.doLazy){window.doLazy[key].appRun()}});showAll();$(document).delegate('[id*="|addGz|"]',"click",function(){var o=$(this).parent();var name=$(this).attr("id").replace(/\|.+\|/,"");o.hide();$.ajax({url:getBasePath()+"/ajax/atten/add.jsp",data:"toUser="+encodeURIComponent(name)+"&m="+$.now(),async:true,dataType:"json",type:"get",success:function(req){if(req&&req.data&&req.data.type=="success"){o.empty().html(name).show();showAttr(o)}else{if(req.data.value=="login"){$.alert("登录超时，请您先登录")}else{if(req.data.value=="more"){$.alert("您的关注人数已达到上限，无法关注")}else{if(req.data.value=="over"){$.alert("您已经关注过该用户")}else{if(req.data.value=="false"){$.alert("关注操作失败，请稍后重试")}}}}o.show()}},error:function(req){$.alert("服务器繁忙，请稍后再试");o.show()}});return false}).delegate('[id*="|delGz|"]',"click",function(){var name=$(this).attr("id").replace(/\|.+\|/,"");var o=$(this).parent().parent();o.hide();$.ajax({url:getBasePath()+"/ajax/atten/del.jsp",data:"toUser="+encodeURIComponent(name)+"&m="+$.now(),async:true,dataType:"json",type:"get",success:function(req){if(req&&req.data&&req.data.type=="success"){o.empty().html(name).show();
showAttr(o)}else{if(req.data.value=="login"){$.alert("登录超时，请您先登录")}else{if(req.data.value=="false"){$.alert("关注操作失败，请稍后重试")}}o.show()}},error:function(req){$.alert("服务器繁忙，请稍后再试");o.show()}});return false});_moveObjId=null;$(document).delegate('[id*="|card|"],[id*="|mcard|"],[id*="|ccard|"],[id*="|vcard|"]',"mouseover",function(){_moveObjId=this;doShowCardTask=setTimeout("showNameCard()",800)}).delegate('[id*="|card|"],[id*="|mcard|"],[id*="|ccard|"],[id*="|vcard|"]',"mouseout",function(){clearTimeout(doShowCardTask)});$("body").mouseover(function(event){var e=event.srcElement||event.target;var p=$(e).parents("#cardSpan");if(p.length>0||e==_moveObjId){return}$("#cardSpan").remove()})});function doHideObj(){$(window).scroll()}function lazyObj(os,runfun,t){this.id="a"+new Date().getTime();this.time=t||500;this.baselist=os||[];this.isAdding=false;this.isRun=true;this.runList=new Array();this.addRunValue=function(arr){for(var i=0;i<arr.length;i++){this.runList.push(arr[i])}if(!this.isRun){this.run()}};this.appRun=function(){if(document.documentElement&&this.baselist&&!this.isAdding){this.isAdding=true;for(var i=0;i<this.baselist.length;i++){var o=$(this.baselist[i]);var objTop=o.offset().top;var top=$(document).scrollTop();var winH=$(window).height();var isVisible=o.is(":visible")||o.parent().is(":visible");if((winH+top>objTop)&&top<=(objTop+o.height())&&isVisible){this.baselist.splice(i,1);i--;this.runList.push(o)}}this.isAdding=false}};this.run=function(){var o=this.runList.shift();if(o){runfun(o)}if(this.baselist.length>0||o||this.runList.length>0){window.setTimeout("window.doLazy."+this.id+".run()",this.time)}else{isRun=false}};this.run();this.appRun()}var cardInfo={userCard:{action:getBasePath()+"/nameCard/userCard.jsp?userName="},medalCard:{action:getBasePath()+"/nameCard/medalCard.jsp?cid="},cirCard:{action:getBasePath()+"/nameCard/vcirCard.jsp?id="},vbCard:{action:getBasePath()+"/nameCard/vbarCard.jsp?id="}};function showNameCard(){var o=$(_moveObjId);var cInfo;if(o.attr("id").indexOf("|mcard|")>-1){cInfo=cardInfo.medalCard}else{if(o.attr("id").indexOf("|ccard|")>-1){cInfo=cardInfo.cirCard}else{if(o.attr("id").indexOf("|vcard|")>-1){cInfo=cardInfo.vbCard}else{cInfo=cardInfo.userCard}}}var array=$(o).offset();$("body").children(":last-child").after("<span id='cardSpan' style='position:absolute;z-index:900;background-color:#ffffff;visibility:hidden'></span>");$("#cardSpan").load(cInfo.action+encodeURIComponent(o.attr("id").replace(/\|.+\|/,"")),function(){var ifm=findIframe(o.attr("id"));var cardWidth=$("#cardSpan").width();var cardHeight=$("#cardSpan").height();var top=array.top+(ifm==null?0:$(ifm).offset().top);if(top-$(document).scrollTop()>cardHeight){top=top-cardHeight+1}else{top=top+o.height()-1}var left=array.left+(ifm==null?0:$(ifm).offset().left);if($(parent.window).width()-(left-$(parent.window).scrollLeft())<cardWidth){left=left-cardWidth+o.width()}if(ifm){$("body").children(":last-child").after($("#cardSpan"));$("#cardSpan").remove()}$("#cardSpan").css({top:top+"px",left:left+"px",visibility:"visible"})})}function cardHover(os){os=$(os);if(os.length<1){return}for(var i=0;i<os.length;i++){os.get(i).id="|card|"+os.get(i).id}}function setToDo(funs){window.todoMethod=funs}function doToDo(){if(window.todoMethod){window.todoMethod();window.todoMethod=null;return true}return false}function showAll(){$(function(){window.setTimeout("showHeadPic()",150);window.setTimeout("showMedalPic()",180);window.setTimeout("showAttr()",200);window.setTimeout("showLoad()",220);window.setTimeout("viewDo()",240)})}function exitLogin(){$.ajax({type:"post",url:"ExitAction.action",async:true,success:function(v){$.alert("退出成功");location.reload()}});return false}function findIframe(id){var ifms=$("iframe");for(var i=0;i<ifms.length;i++){if($(ifms[i].contentDocument).find("#"+id.replace(/\|/g,"\\|")).length>0){return ifms[i]}}return null}function waitDiv(time,isImg){time=time||5000;if(isImg!==false){isImg=true}return showDialog(isImg?"<img src='"+getBasePath()+"/images/loading2.gif'/>":"",true,time)}function closeWDiv(isFast){var doc=$(document.body).length>0?$(document.body):$("body");if(isFast){$("#divDialog",doc).fadeOut(0)}else{$("#divDialog",doc).fadeOut("slow")}clearTimeout(closeThread);$("#divShield",doc).remove()}$(function(){$(document).add($(document)).delegate("img[id*='|dialog|']","click",function(e){var img=this.src;img="<img id='noGreenCard' align='center' src='"+img+"'/>";if(this.alt=="zhuansoo"){showDialog(img,true,undefined,$(window).width())}else{showDialog(img,true)}var e=e||window.event;e.stopPropagation()});$(document).delegate('[id*="|dialog|"]',"mouseover",function(){$(this).parent().css({"cursor":'url("'+getBasePath()+'/images/fd.cur"), pointer'})})});var closeThread;function showDialog(content,flag,waitTime,maxWidth){var win=$(parent.window).length>0?$(parent.window):$(window);
var windowH=$(win).height();var windowW=$(win).width();var maxViewWith=maxWidth!=undefined?maxWidth:700;var shield="<div id='divShield' style='background: #000;position: absolute;z-index: 501;top:0;left:0;display:none'></div>";var ID="divDialog";var hand="cursor:url("+getBasePath()+"/images/sx.cur),pointer;";var div="<div  align='center' id='"+ID+"' style='position:absolute;z-index:999;display:none;"+(content.indexOf("id='noGreenCard'")>-1?hand:"")+"'></div>";var doc=$(document.body).length>0?$(document.body):$("body");var w=$(doc).width();var h=$(doc).height();if($("#"+ID).length>0){$("#"+ID).remove()}$(doc).append(div);$(doc).append(shield);var dialog=$("#"+ID,doc);$(dialog).html(content);if($(dialog,doc).children("img[id='noGreenCard']").get(0)){var img=new Image();img.src=$(dialog,doc).children("img[id='noGreenCard']").attr("src");var noGreenW=img.width;if(noGreenW>maxViewWith){var noGreenH=img.height;noGreenH=Math.round(noGreenH/(noGreenW/maxViewWith));noGreenW=maxViewWith;$(dialog,doc).children("img[id='noGreenCard']").width(noGreenW,noGreenH)}}var dialogH=$(dialog).height();var dialogW=$(dialog).width();if(flag){$("#divShield",doc).width($(top.document).width()).height($(top.document).height()).css({filter:"alpha(Opacity=80)","-moz-opacity":"0.5","opacity":"0.5"}).show()}$(dialog).fadeIn().offset({"top":""+((windowH-dialogH)/2+$(win).scrollTop())+"","left":""+((windowW-dialogW)/2+$(win).scrollLeft())+""});function closeWaitDiv(){if($("#"+ID,doc).length>0){$("#"+ID,doc).fadeOut("slow")}if($("#divShield",doc).length>0){$("#divShield",doc).remove()}$("body").unbind("click",closeWaitDiv)}if(waitTime!==undefined){if(waitTime>0){closeThread=setTimeout(closeWaitDiv,waitTime)}}else{$("body").bind("click",closeWaitDiv)}}function backTop(position){$("body").append("<img id='_backtop' width='30px;' height='30px;' style=\"cursor: pointer;position: fixed;z-index: 99;display:none;\" src='"+getBasePath()+"/images/_top.png'/>");left=position?$(window).width()-position:$(window).width()-200;$("#_backtop").offset({top:$(window).height()+$(document).scrollTop()-100,left:left});if($(document).scrollTop()>0){$("#_backtop").show()}$(window).scroll(function(){left=position?$(window).width()-position:$(window).width()-200;$("#_backtop").offset({top:$(window).height()+$(document).scrollTop()-100,left:left});var top=$(document).scrollTop();if(top>=10){$("#_backtop").show()}else{$("#_backtop").hide()}});$("#_backtop").click(function(){$(document).scrollTop(0)})}function selNav(o1,o2){$(o1).css({"color":"#ff7f1a","text-decoration":"none"});$(o2).css({"color":"#FFF","text-decoration":"none"})}function imgErrFun(e,src){e=e||window.event;var o=e.target||e.srcElement;var num=$(o).attr("lodNum")||0;num++;if(num>2){return false}$(o).attr("lodNum",num);if(num>1){if(src.indexOf("http")>0){src=src.substring(src.indexOf("/",10))}src="http://114.113.229.4/"+src.replace("/NewRoot","")}$(o).attr("src",src)}$.dl=function(config){if(!config){alert("config is required");return false}var url=config.url;if(!url){alert("config.url is required");return false}var clickObj=config.trigger;if(!clickObj){alert("config.trigger is required");return false}var data=config.data?config.data:{};var random_if="dl_"+$.now();$(function(){if(clickObj.selector&&clickObj.selector!="a"){clickObj.wrap("<a id='"+random_if+"'></a>");clickObj=$("#"+random_if)}$("body").append("<iframe style='position:absolute;top: -900px;left: -900px;' name='"+random_if+"'></iframe>");clickObj.attr("href",url+"?"+data).attr("target",random_if)})};$.os={};$.os.msie=navigator.userAgent.indexOf("MSIE")>0;
/**
 * 获取地址栏各个参数，返回json
 * @param {} f
 */
...
$(document).ajaxComplete(function(event,request,settings){
	if(request.responseText && request.responseText.indexOf('nologin_login')> -1){
		alert('登录超时，请重新登录');
		top.location.href='/u/login.jsp';
	}
	//console.log(request.responseText)
	if(request.responseText && request.responseText.indexOf('hasNoPermission')> -1){
		alert('您无此权限');
		//console.log(request.responseText)
		//top.location.href='/u/login.jsp';
	}
...
var objShowBackDiv="<div id='objShowBackDiv' style='background:#000000;position:fixed;display:none;filter:alpha(Opacity=50);-moz-opacity:0.5;opacity:0.5'></div>";
var openDivNum=0;
var showObj=function(id,isAuto){
	var auto={};
	...
	if(auto.top||auto.left){
		auto.auto=true;
	}
	openDivNum++;
	$('#'+id).find('*').andSelf().css({"z-index":9999999+openDivNum}).show();
	if(auto.auto){
		var h=($(window).height()-$('#'+id).outerHeight())/2;
		var w=($(window).width()-$('#'+id).outerWidth())/2;
		var cssObj={'position':'fixed'};
		if(auto.top)cssObj.top=h;
		if(auto.left)cssObj.left=w;
		$('#'+id).css(cssObj);
	}
	if($('#objShowBackDiv').length<1){
		$('#'+id).after(objShowBackDiv);
		$('#objShowBackDiv').offset({top:0,left:0}).css({'height':$(document).height(),'width':$(document).width(),"z-index":9999998+openDivNum});
	}
	$('#objShowBackDiv').show();
}
var hideObj=function(id){
	...
	$('#'+id).hide();
	if(openDivNum<=0){
		$('#objShowBackDiv').hide();openDivNum=0;
	}
}
//多点登陆
$(function(){
	try{
		$.get('/ajax/vbar/c0ef4307f8ff9b9beded5c3967c53e9a.jsp',function(v){
			v = v.replace(/^\\s+|\\s+$/g,'');
			if(v.length>=32){
				$.getScript(getProt()+'//vb.qb178.com/vbar/cascadeLogin.jsp?para='+v,function(w){
					$.getScript(getProt()+'//www.zhuansoo.com/vbar/cascadeLogin.jsp?para='+v,function(w){
